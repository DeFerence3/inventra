...
class GetOrderItemsByPurchaseOrdersUseCase @Inject constructor(
    private val purchaseRepo: PurchaseRepo
) {
    operator fun invoke(
        poUUIDs: List<String>
    ): Flow<RequestState<List<OrderItem>>> = flow {
        ...
        try {
            ...
            val deferredResponse = purchaseRepo.getItemsByPurchaseOrders(poUUIDs)
